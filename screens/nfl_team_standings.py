#!/usr/bin/env python3
"""Standalone NFL team standings screens using Bears-style layout."""
import os
import time
...
def draw_standings_screen1(display, rec, logo_path, transition=False):
    """Screen 1: logo, W/L/T, rank, and pct (no games-back concepts)."""
    if not rec:
        return None
    ...
    ties = lr.get("ties")
    ties_txt = f" T: {ties}" if ties not in (None, 0, "0") else ""
    wl_txt = f"W: {wins} L: {losses}{ties_txt}"
    ...
    pct = lr.get("pct")
    pct_txt = f"Pct: {pct}" if pct not in (None, "") else None
    ...
    if pct_txt:
        lines.append((pct_txt, FONT_STAND1_GB_VALUE))
...
def draw_standings_screen2(display, rec, logo_path, transition=False):
    """Screen 2: logo + overall record and basic splits."""
    if not rec:
        return None
    ...
    ties_part = f"-{ties}" if ties not in (None, 0, "0") else ""
    rec_txt = f"{wins}-{losses}{ties_part} ({pct})"
    ...
    def find_split(t):
        for sp in splits:
            if sp.get("type", "").lower() == t.lower():
                ...
                ties_val = sp.get("ties")
                ties_suffix = f"-{ties_val}" if ties_val not in (None, 0, "0") else ""
                return f"{wins}-{losses}{ties_suffix}"
        return "-"

    items = [
        f"Streak: {rec.get('streak', {}).get('streakCode', '-')}",
        f"Last 5: {find_split('lastFive')}",
