...
└─ fonts/
   ...
   ├─ DejaVuSans.ttf
   ├─ DejaVuSans-Bold.ttf
   └─ HWYGNRRW.TTF               # Travel time screen font (bundle your licensed copy)
```
...
9. Every sixth pass, cycle through `NHL Div Standings West Overview`, `NHL Div Standings Overview East`, `NHL Div Standings West`.
10. Every eighteenth pass (starting at phase 12), show `NHL Div Standings East`.
...
- **Fonts:** copy `TimesSquare-m105.ttf`, `DejaVuSans.ttf`, and `DejaVuSans-Bold.ttf` into `fonts/`; install `fonts-noto-color-emoji` to provide the emoji font.
